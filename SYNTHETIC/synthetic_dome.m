...
% Synopsis: S=synthetic_antecline([200,150,100]); S output synthetic cube with a
% dome made from 2 first order gaussians convolved
% with a ricker to look like seismic.
function S=synthetic_dome(isize)
...
for jj=1:nj 
        ...
        timej=round(.25*ni*(1-exp(-.5*scaled^2))); % a shifted gaussian        
        
        for kk=1:nk
        scaled=(kk-1)/(nk-1); %in [0 1] now
        scaled=scaled-0.5; %in [-.5 .5] now
        scaled=8*scaled; %in[-4 4]
        timek=round(.25*ni*(1-exp(-.5*scaled^2))); % a shifted gaussian  
            
            for mm=1:size(events,1) %looping on all the reflectivity events
                pos=timej+timek+events(mm,1);
